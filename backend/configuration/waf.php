...
class aWAF {
    ...
    function vulnDetectedHTML($Method, $BadWord, $DisplayName, $TypeVuln) {
        ...
        echo 'There is a problem with your request, please try again (' . $BadWord . ', ' . $TypeVuln . ').';
        die(); // Block request.
    }
    ...
    public function sqlCheck($Value, $Method, $DisplayName) {
        ...
        // Retrieve the list of bad words for SQL injection detection.
        $BadWords = $this->getArray('SQL');

        // Check if any bad word is present in the input value.
        foreach ($BadWords as $BadWord) {
            // Check if $BadWord exists in $Value (case insensitive).
            if (stripos($Value, $BadWord) !== false) {
                // String contains a potential SQL injection keyword.
                $this->vulnDetectedHTML($Method, $BadWord, $DisplayName, 'SQL Injection');
            }
        }
    }
