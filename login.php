...
<body>
            ...
            <div class="flex items-center w-full max-w-md px-6 mx-auto lg:w-2/6">
                <div class="flex-1 rounded-lg p-6" style="background-color: rgb(17 24 39 / 0.4);">
                    ...
                    <div class="mt-8">
                        ...
                        <div class="mt-6">
                            <button
                                id="signinButton" onclick="SignIn()" disabled class="w-full px-4 py-2 tracking-wide text-white transition-colors duration-200 transform bg-[#8B78EA] rounded-md hover:bg-blue-400 focus:outline-none focus:bg-blue-400 focus:ring focus:ring-blue-300 focus:ring-opacity-50">
                                Loading..
                            </button>
                        </div>
                        ...
                        <p class="mt-6 text-sm text-center text-gray-400">Don&#x27;t have an account yet? <a href="/nebulanuke/register" class="text-blue-500 focus:outline-none focus:underline hover:underline">Sign up</a>.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
             // Mocking Turnstile behavior for local testing
const turnstileKeys = {
    'localhost': "0x3355E846D943e272be0688a1DE8E08d50A9AA09B",
};

// Simulate the turnstile.ready function
function simulateTurnstileReady(callback) {
    // Simulate asynchronous behavior with a timeout
    setTimeout(callback, 100); // Adjust timeout as needed
}

// Simulate the turnstile.render function
function simulateTurnstileRender(containerSelector, options) {
    // Simulate captcha rendering and callback
    const sitekey = turnstileKeys[window.location.host];
    if (sitekey) {
        // Simulate captcha rendering completion after some time
        setTimeout(() => {
            const token = '0x3355E846D943e272be0688a1DE8E08d50A9AA09B'; // Replace with actual mock token if needed
            options.callback(token);
        }, 500); // Simulate delay for rendering, adjust as needed
    } else {
        console.error('Sitekey not found for local host');
    }
}

// Usage in your script
simulateTurnstileReady(function() {
    simulateTurnstileRender('#captcha_container', {
        retry: 'auto',
